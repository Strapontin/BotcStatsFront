...
export default function CreateGame() {
  ...
  const gameDraftId = Number(searchParams.get("gameDraftId")) || NaN;
  const { data: gameDraft } = useGetGameDraftById(gameDraftId);
  const [oldGameDraft, setOldGameDraft] = useState(gameDraft);

  useEffect(() => {
    if (
      (gameDraft !== oldGameDraft || gameKey === 0) &&
      gameDraft?.id != undefined
    ) {
